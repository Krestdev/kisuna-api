...
export class MeController {
  ...
  @Get('attendance')
  @ApiOperation({ summary: 'Get employee attendance records with filters and pagination' })
  @ApiQuery({ name: 'statuses', required: false, description: 'Comma-separated statuses (PRESENT,LATE,FIELD)', example: 'PRESENT,LATE' })
  @ApiQuery({ name: 'startDate', required: false, description: 'Start date filter', example: '2025-11-01' })
  @ApiQuery({ name: 'endDate', required: false, description: 'End date filter', example: '2025-11-30' })
  ...
  @ApiResponse({ status: 201, description: 'Field presence marked successfully' })
  @ApiResponse({ status: 422, description: 'Validation error - missing required fields' })
  ...
  @ApiQuery({ name: 'limit', required: false, description: 'Number of records', example: 5 })
  @ApiResponse({ status: 200, description: 'Recent leaves retrieved successfully' })
  ...
  @Get('leaves')
  @ApiOperation({ summary: 'Get employee leave requests with filters and pagination' })
  @ApiQuery({ name: 'status', required: false, description: 'Leave status filter', example: 'APPROVED' })
  @ApiQuery({ name: 'startDate', required: false, description: 'Start date filter', example: '2026-01-01' })
  @ApiQuery({ name: 'endDate', required: false, description: 'End date filter', example: '2026-12-31' })
  @ApiQuery({ name: 'page', required: false, description: 'Page number', example: 1 })
  @ApiQuery({ name: 'limit', required: false, description: 'Items per page', example: 10 })
  ...
  @Post('leaves')
  @ApiOperation({ summary: 'Submit a new leave request with optional justification file' })
  ...
  @ApiBody({
    schema: {
      ...
      properties: {
        type: { type: 'string', enum: ['ANNUAL', 'SICK', 'MATERNITY', 'PATERNITY', 'UNPAID', 'OTHER'], example: 'ANNUAL' },
        ...
        justificatif: { type: 'string', format: 'binary', description: 'Optional justification file' },
      },
    },
  })
